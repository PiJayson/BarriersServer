...
/* returns index of process that coresponds to given name */
int get_proc_with_name(const char* proc_name) {
	for (int i = 0; i < NR_PROCS; i++) {
		if (strcmp(mproc[i].mp_name, proc_name) == 0 && mproc[i].mp_flags & IN_USE) return i;
	}
	return -1;
}
 ...
 *				tell_bs				 	     *
 *===========================================================================*/
void tell_bs(rmp, m_ptr)
struct mproc *rmp;
message *m_ptr;
{
/* Send a request to BS, without blocking. */
  int r;
  endpoint_t pt;

  r = get_proc_with_name(BARRIER_SERVER_NAME);	/* BARRIER_SERVER_NAME = "barriers" */
  if (r == -1) return;			/* we want to ignore if barriers still does not exist */

  pt = mproc[r].mp_endpoint;

  r = asynsend3(pt, m_ptr, AMF_NOREPLY);
}
